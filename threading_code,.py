...
def main():
    for cores in cores_range:
        ...
        for _ in range(num_runs):
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=cores) as executor:
                pairs = [(molecules[i], molecules[j]) for i in range(len(molecules)) for j in range(i+1, len(molecules))]
                results = list(executor.map(identify_mcs_pair, pairs))
    ...
    plt.title('Number of CPU Cores vs Average Time Taken (Threaded)')
